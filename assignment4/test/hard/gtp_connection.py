...
#==========================================================================================
    ...
    def genmove_cmd(self, args):
        ...
        count = self.count_op(color)
        if count >2:#more than 2
            #move = self.get_move_score_based(color)
            # try:
            #     signal.alarm(int(self.timelimit) - 1)
            #     self.sboard = self.board.copy()
            ...
            move = self.good_start_2(color)
        else:
            move = self.good_start_1(color)
    ...
    def count_op(self,color):
        ...
        return count_oppoent

    def good_start_1(self,color):
        board2D = GoBoardUtil.get_twoD_board(self.board)
        move = None
        mid = board2D[3][3]
        op = 3 - color 
    ...
    def good_start_2(self,color):
        op = 3 - color 
        board2D = GoBoardUtil.get_twoD_board(self.board)
        size = self.board.size
        move = None
        if board2D[3][3] == color:
            hasMid = True
        else:
            hasMid = False

        if hasMid:
            if board2D[2][4] == color:
                if board2D[4][2] == 0 and board2D[1][5] == 0:
                    move = coord_to_point(5,3,size)
                else:
                    move = coord_to_point(4,5,size)

            elif board2D[4][2] == color:
                if board2D[2][4] == 0 and board2D[5][1] == 0:
                    move = coord_to_point(3,5,size)
                else:
                    move = coord_to_point(4,3,size)

            elif board2D[2][2] == color:
                if board2D[1][1] == 0 and board2D[4][4] == 0:
                    move = coord_to_point(5,5,size)
                else:
                    move = coord_to_point(4,3,size)

            elif board2D[4][4] == color:
                if board2D[2][2] == 0 and board2D[5][5] == 0:
                    move = coord_to_point(3,3,size)
                else:
                    move = coord_to_point(4,5,size)

        else:
            if board2D[2][2] == op and board2D[4][4] == 0:
                move = coord_to_point(5,5,size)
            elif board2D[2][3] == op and board2D[4][3] == 0:
                move = coord_to_point(5,4,size)
            elif board2D[2][4] == op and board2D[4][2] == 0:
                move = coord_to_point(5,3,size)
            elif board2D[3][2] == op and board2D[3][4] == 0:
                move = coord_to_point(4,5,size)
            elif board2D[3][4] == op and board2D[3][2] == 0:
                move = coord_to_point(4,3,size)
            elif board2D[4][2] == op and board2D[2][4] == 0:
                move = coord_to_point(3,5,size)
            elif board2D[4][3] == op and board2D[2][3] == 0:
                move = coord_to_point(3,4,size)
            elif board2D[4][4] == op and board2D[2][2] == 0:
                move = coord_to_point(3,3,size)
            elif board2D[2][4] == 0:
                move = coord_to_point(3,5,size)
            elif board2D[4][2] == 0:
